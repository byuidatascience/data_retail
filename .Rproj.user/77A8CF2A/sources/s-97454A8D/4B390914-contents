...
for (st in states) {
  ...
  for (name in WeatherStation$Name){
    ...
    print(name) # indicator for how it's moving 
  }
...
# TMY3 weather set for Los Angeles
LosAngeles_tmy3 <- CA_tmy3 %>% 
...
LosAngeles_StoreSim <- read_csv("raw_data/Energry Plus/los_angeles.csv") 
LosAngeles_StoreSim <- LosAngeles_StoreSim %>% 
...
LosAngeles <- left_join(LosAngeles_StoreSim, LosAngeles_tmy3) 
LosAngeles <- read_csv("derived_data/joined_data/LosAngeles.csv") %>% 
  select(`Date (MM/DD/YYYY)`, `Time (HH:MM)`, Date, Time, month, day, hour, StationName, everything())

# Write joined data to csv
write_csv(LosAngeles, "LosAngeles.csv")